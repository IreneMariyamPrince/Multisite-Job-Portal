...
const Regions = () => {
  ...
  const [totalActiveRegions, setTotalActiveRegions] = useState();
  const [totalPendingRegions, setTotalPendingRegions] = useState();
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        setTotalActiveRegions(response.counts.totalActiveRegions);
        setTotalPendingRegions(response.counts.totalPendingRegions);
      } else {
        console.log('error');
      }
    } catch (error) {
  ...
  const handleDeleteConfirmation = async id => {
    try {
      ...
      await regionApi.regionDelete(id);
  ...
  return (
    <>
      <RegionInfoCards
        totalCount={count}
        totalActiveRegions={totalActiveRegions}
        totalPendingRegions={totalPendingRegions}
      />
          ...
          <div className='card my-4 mb-0'>
            <div className='card-header p-0 position-relative mt-n4 mx-3 z-index-2'>
              <div className='bg-gradient-primary shadow-primary border-radius-lg pt-4 px-3 pb-3 d-flex align-items-center justify-content-between'>
                <h6 className='text-white text-capitalize ps-3 mb-0'>Region List</h6>
              ...
              <div className='card-body px-0 pb-2'>
                <div className='table-responsive p-0'>
                  <table className='table align-items-center mb-0'>
                    ...
                    <tbody>
                      {regionData?.data?.map((data, index) => {
                        const { firstName, email, userStatus, mobile, createdAt } = data;
                        const { logo } = data.RegionalInfo;
                        const { webInfoWebUrl } = data.WebInfo;
                              ...
                              <div className='d-flex px-2 py-1'>
                                ...
                                <div className='d-flex flex-column justify-content-center'>
                                  ...
                                  <p className='text-xs text-secondary mb-0'>
                                    {' '}
                                    {webInfoWebUrl === null ? 'Not defined yet' : webInfoWebUrl}
                                  </p>
                                </div>
                              </div>
                            </td>
                            <td>
                              <p className='text-xs font-weight-bold mb-0'>{mobile}</p>
                      ...
                      {regionData && regionData.data && regionData.data.length === 0 && (
                        <tr>
